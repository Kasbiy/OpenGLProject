...
static LRESULT CALLBACK WindowEvenHandler(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_MOUSEMOVE:
    {
        if (window->GetMouseButtonDown())
        {
            ...
            {
                window->SetCameraMovement(mousePos.x - centerPos.x, centerPos.y - mousePos.y);
...
bool Window::InitWindow(const std::string_view& name, uint16_t width, uint16_t height)
{
    if (!m_IsInitialized)
    {
        m_Name = name;
...

        {
...

            {
                ...
                {
                    int32_t contextFlags = GL_NONE;

                    // Set to true by default feel free to set t to false
                    ...
                    if (WGLEW_ARB_create_context_no_error && !useDebugContext)
                    {
...
void Window::Resize(const WORD &width, const WORD &height)
{
    if (m_Width != width || m_Height != height)
    {
        ...
        m_Camera->ComputeProjection(width, height);
